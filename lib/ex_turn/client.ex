  ...
  * `:error` - an error has occured and the client cannot be used anymore.
  ...
  def allocate(%__MODULE__{state: :new} = client) do
    req =
      ...
      |> Message.new([%RequestedTransport{protocol: :udp}])
  ...
  @spec send(t(), addr(), binary()) :: {:send, addr(), binary(), t()}
  ...
  defp do_handle_stun_message(
    ...
    case Message.authenticate(resp, client.key) do
      :ok ->
        ...
        addr_channel = Map.put(client.addr_channel, peer_addr, channel_number.number)
        channel_addr = Map.put(client.channel_addr, channel_number.number, peer_addr)
